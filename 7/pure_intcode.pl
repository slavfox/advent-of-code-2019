...
intcode_eval(
    ...
    list_nth_replace(Prog, Addr, Input, NewProg).
intcode_eval(
    4,
    [Arg],
    Ins, Ins,
...
intcode(_, [halt|Prog], Prog, _, Outputs, Outputs).
intcode(IPtr0, Prog, Result, Inputs, OutputAccum, Outputs) :-
    ...
    intcode_eval(
        ...
        Params,
        Inputs, NewInputs,
        OutputAccum, NewOutputAccum,
        IPtr1, IPtr2,
        Prog, NewProg
    ),
    intcode(IPtr2, NewProg, Result, NewInputs, NewOutputAccum, Outputs).

intcode(Prog, Result, Inputs, Outputs) :-
    intcode(0, Prog, Result, Inputs, [], Outputs).
